  ...
  <v-dialog :width="width" v-model="dialogValue">
    <v-card>
      ...
      <v-card-text class="text-center ma-md-5">
        <div class="text-info">
          <i class="ph ph-paper-plane-tilt ph-4x" />
        </div>
        <div class="mt-4">
          <h4 class="text-h6 font-weight-bold">
            <slot name="title"> {{ $t("t-dialog-title-confirm-employee-extract-notification") }}</slot>
          </h4>
          <p class="text-muted mx-4 mb-0 text-subtitle-1">
            <slot name="text">
              {{ $t("t-dialog-text-confirm-employee-extract-notification") }}
            </slot>
          </p>
        </div>
      </v-card-text>
      <v-card-actions class="d-flex justify-center mt-4 mb-7">
        ...
        >
          {{ $t("t-yes-send-employee-extract-notification") }}
        </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
