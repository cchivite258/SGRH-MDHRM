...
export default class PositionService extends HttpService {
  ...
  async updatePosition(id: string, positionData: PositionInsertType): Promise<ServiceResponse<PositionListingForListType>> {
    try {
      ...
      const response = await this.put<ServiceResponse<PositionListingForListType>>(`/administration/positions/${id}`, payload);
      return {
        status: 'success',
        data: response.data
      };
