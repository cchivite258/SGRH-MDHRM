...
import { ProviderTypeInsert } from "@/components/baseTables/providerType/types";
...
const prop = defineProps({
  ...
  data: {
    type: Object as PropType<ProviderTypeInsert>,
    required: true,
  },
  error: {
    type: String,
...
const requiredRules = {
  name: [(v: string) => !!v?.trim() || t("t-please-enter-name-institution-type")],
};
...
<template>
  <v-dialog v-model="dialogValue" width="500" scrollable>
    <v-form ref="form" @submit.prevent="onSubmit">
      <Card :title="isCreate ? $t('t-add-provider-type') : $t('t-edit-provider-type')" title-class="py-0"
