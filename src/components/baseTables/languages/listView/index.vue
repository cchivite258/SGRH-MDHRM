    ...
    <DataTableServer v-model="selectedLanguages" v-model:page="currentPage"
      ...
      <template #body="{ items }">
        <tr v-for="item in items as LanguagesListing[]" :key="item.id" class="base-table-listing-page__row">
          ...
          <td data-label="Acção" class="base-table-listing-page__actions-cell"><TableActionMenu @onEdit="onCreateEditClick(item)" @onView="onViewClick(item)" @onDelete="onDelete(item.id)" /></td>
        </tr>
      </template>
