...
const sortedRows = computed(() => {
  return [...(props.report || [])].sort((a, b) => (Number(b.totalBilled || 0) - Number(a.totalBilled || 0)));
});

const { page, itemsPerPage, paginatedRows: paginatedRows } = useReportPreviewPagination(sortedRows);
...
const handleExport = async (type: "pdf" | "excel" | "csv") => {
  try {
    ...
    const fileNamePrefix = locale.value === "en"
      ? "total-billed-medical-assistance"
      : "total-facturado-assistencia-medica";
    const fileName = `${fileNamePrefix}-${new Date().toISOString().split("T")[0]}`;

    switch (type) {
      case "pdf":
        await TotalBilledMedicalAssistanceReportExporter.exportToPDF(props.report, userName.value, { fileName });
        break;
      case "excel":
        await TotalBilledMedicalAssistanceReportExporter.exportToExcel(props.report, userName.value, { fileName });
        break;
      case "csv":
        await TotalBilledMedicalAssistanceReportExporter.exportToCSV(props.report, userName.value, { fileName });
        break;
    }
  } catch (error) {
      ...
      <div class="d-flex justify-space-between align-center mb-4">
        <div>
          ...
          <div class="text-caption text-grey mt-1">
            {{ $t("t-report") }} #100007 - {{ $t("t-report-100007-title") }}
          </div>
        </div>
    ...
    <v-card variant="outlined" class="mb-6" elevation="0">
      ...
      <div class="table-responsive">
        <v-table density="comfortable" hover>
          ...
          <tbody>
            <tr v-for="(row, i) in paginatedRows" :key="i" class="table-row">
              <td class="pa-4">{{ row.serviceProviderName || "-" }}</td>
              <td class="pa-4">{{ row.serviceProviderTypeName || "-" }}</td>
              <td class="text-right pa-4">{{ row.totalEmployees || 0 }}</td>
              <td class="text-right pa-4 font-weight-medium">{{ amountFormate(row.totalBilled || 0) }} MT</td>
            </tr>
          </tbody>
          <tfoot>
            <tr class="total-row">
