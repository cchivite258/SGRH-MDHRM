...
import { employeeFrequencyTrendReportService } from "@/app/http/httpServiceProvider";
import { getApiErrorMessages } from "@/app/common/apiErrors";
import type { EmployeeFrequencyTrendFilterType } from "@/components/ammReports/types";
...
const onSubmit = async (exportType: ExportType = "pdf") => {
  ...
  try {
    const payload: EmployeeFrequencyTrendFilterType = {
      contractId: contractId.value,
      coveragePeriodId: coveragePeriodId.value,
    };

    console.log("[EmployeeFrequencyTrendReport] payload sent to API:", payload);

    const response = await employeeFrequencyTrendReportService.createReport(payload);
    console.log("[EmployeeFrequencyTrendReport] API response:", response);
    if (response.status === "error") {
      const messages = getApiErrorMessages(response.error, t("t-error-generating-report"));
      errorMsg.value = messages[0] || t("t-error-generating-report");
    ...
    if (exportType === "pdf") {
      await EmployeeFrequencyTrendReportExporter.exportToPDF(response.data, userName.value, { fileName });
    } else if (exportType === "excel") {
      await EmployeeFrequencyTrendReportExporter.exportToExcel(response.data, userName.value, { fileName });
    } else {
      await EmployeeFrequencyTrendReportExporter.exportToCSV(response.data, userName.value, { fileName });
    }

    emit("update:modelValue", false);
    toast.success(t("t-file-generated-successfully", { format: exportType.toUpperCase() }));
  } catch (error: any) {
    const messages = getApiErrorMessages(error, t("t-error-generating-report"));
    errorMsg.value = messages[0] || t("t-error-generating-report");
    messages.forEach((message) => toast.error(message));
  } finally {
