...
const groupedRows = computed<GroupedRow[]>(() => {
  ...
  const providersSorted = [...(props.report || [])].sort((a, b) => {
    const totalA = (a.details || []).reduce((sum, item) => sum + (item.totalBilled || 0), 0);
    const totalB = (b.details || []).reduce((sum, item) => sum + (item.totalBilled || 0), 0);
...
const { page, itemsPerPage, paginatedRows: paginatedGroupedRows } = useReportPreviewPagination(groupedRows);
...
const handleExport = async (type: "pdf" | "excel" | "csv") => {
  try {
    ...
    const fileNamePrefix = locale.value === "en"
      ? "top-service-types-by-service-provider"
      : "tipos-servico-mais-realizados-por-provedor-de-servico";
    const fileName = `${fileNamePrefix}-${new Date().toISOString().split("T")[0]}`;

    switch (type) {
      case "pdf":
        await TopServiceTypesByClinicReportExporter.exportToPDF(props.report, userName.value, { fileName });
        break;
      case "excel":
        await TopServiceTypesByClinicReportExporter.exportToExcel(props.report, userName.value, { fileName });
        break;
      case "csv":
        await TopServiceTypesByClinicReportExporter.exportToCSV(props.report, userName.value, { fileName });
        break;
    }
  } catch (error) {
  ...
  <v-container class="py-8 px-4" max-width="1200px">
      ...
      <div class="d-flex justify-space-between align-center mb-4">
        <div>
          ...
          <div class="text-caption text-grey mt-1">
            {{ $t("t-report") }} #100004 - {{ $t("t-top-service-types-by-service-provider") }}
          </div>
        </div>
        ...
        <v-card variant="outlined" class="pa-4 h-100" elevation="0">
          <div class="d-flex align-center mb-3">
            ...
            <div>
              ...
              <div class="text-h6 font-weight-bold">{{ totalProviders }}</div>
            </div>
          </div>
          ...
          <div class="text-caption text-grey">
            <div class="d-flex justify-space-between my-1">
              ...
              <span class="font-weight-medium">{{ detailRows.length }}</span>
            </div>
          </div>
        </v-card>
      </v-col>
    ...
    <v-card variant="outlined" class="mb-6" elevation="0">
      <v-card-title class="d-flex align-center pa-4">
        ...
        <span class="text-body-1 font-weight-medium">{{ $t("t-tsbc-detailed-section") }}</span>
        <v-chip size="small" variant="outlined" class="ml-3">
          {{ detailRows.length }} {{ $t("t-spr-procedures-performed") }}
        </v-chip>
      </v-card-title>
      ...
      <div class="table-responsive">
        <v-table density="comfortable" hover>
          ...
          <tbody>
            <tr v-for="(row, i) in paginatedGroupedRows" :key="i" class="table-row">
              <template v-if="row.type === 'provider'">
                <td colspan="3" class="pa-4 provider-separator font-weight-bold">
                  <v-icon size="small" class="mr-2">mdi-hospital-box</v-icon>
                  {{ $t("t-service-provider") }}: {{ row.providerName }}
                </td>
              </template>
              <template v-else>
        ...
        <ReportPreviewPagination
          ...
          :total-items="groupedRows.length"
        />
      </div>
    </v-card>
