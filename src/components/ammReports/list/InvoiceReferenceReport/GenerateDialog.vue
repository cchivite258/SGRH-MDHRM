...
const onSubmit = async (exportType: ExportType = "pdf") => {
  ...
  try {
    ...
    const prefix = locale.value === "en" ? "invoice-reference-report" : "relatorio-facturas-por-referencia";
    const fileName = `${prefix}-${new Date().toISOString().split("T")[0]}`;
    if (exportType === "pdf") {
      await InvoiceReferenceReportExporter.exportToPDF(response.data, userName.value, { fileName });
    } else if (exportType === "excel") {
      await InvoiceReferenceReportExporter.exportToExcel(response.data, userName.value, { fileName });
    } else {
      await InvoiceReferenceReportExporter.exportToCSV(response.data, userName.value, { fileName });
    }
