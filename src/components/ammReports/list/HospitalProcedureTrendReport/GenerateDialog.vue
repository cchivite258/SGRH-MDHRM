...
const onSubmit = async (exportType: ExportType = "pdf") => {
  ...
  try {
    ...
    if (exportType === "pdf") {
      await HospitalProcedureTrendReportExporter.exportToPDF(response.data, userName.value, { fileName });
    } else if (exportType === "excel") {
      await HospitalProcedureTrendReportExporter.exportToExcel(response.data, userName.value, { fileName });
    } else {
      await HospitalProcedureTrendReportExporter.exportToCSV(response.data, userName.value, { fileName });
    }
