...
const onSubmit = async (exportType: ExportType = "pdf") => {
  ...
  try {
    ...
    if (exportType === "pdf") {
      await EmployeeHealthPlanLimitsTrendReportExporter.exportToPDF(response.data, userName.value, { fileName });
    } else if (exportType === "excel") {
      await EmployeeHealthPlanLimitsTrendReportExporter.exportToExcel(response.data, userName.value, { fileName });
    } else {
      await EmployeeHealthPlanLimitsTrendReportExporter.exportToCSV(response.data, userName.value, { fileName });
    }
