<template>
    <v-card class="mb-4">
        <v-card-text class="pa-4">
            <!-- 🔍 Pesquisa Global -->
            <QuerySearch v-model="globalSearch" :placeholder="$t('t-clinic-search')"
...
const filterableFields = ref([
    ...
    { text: t('t-person-of-contact-phone-1'), value: 'personOfContactEmail1', type: 'text' },
...
const onGlobalSearch = () => {
    clinicStore.setGlobalSearch(globalSearch.value);
...
const addFilter = () => {
    advancedFilters.value.push({
        ...
        value: isBooleanField(filterableFields.value[0].value) ? true :
            isEnumField(filterableFields.value[0].value)
    });
};
...
const applyAdvancedFilters = async () => {
    ...
    try {
        ...
        clinicStore.setLogicalOperator(logicalOperator.value);
        clinicStore.setAdvancedFilters(filtersToSend);
